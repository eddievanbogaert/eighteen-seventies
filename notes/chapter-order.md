...
| # | chapter | provisional date | home thread |
| --- | --- | --- | --- |
...
The brief listed the tentpoles as chronological. Two look out of sequence on
recalled dates, and the files follow the recalled chronology rather than the
listed order. Both moves are reversible and both need checking first.

1. **Descent of Man now precedes Chicago.** The brief listed the fire first.
   Recall puts Darwin's publication early in 1871 and the fire in the autumn.
   If that is right, the listed order is wrong; if recall is wrong, swap them
   back. Verify Q02-1 and Q03-1 before this is settled.
2. **The Indian famine moved earlier.** The brief listed it near the end,
   after Chief Joseph. Its span begins earlier than that placement implies, so
   it now sits just before the 1876 election. This one is genuinely arguable
   rather than simply a date question — see the next section.

Nothing else was moved.

## Open ordering problems

**Multi-year chapters break a strict chronology.** Four chapters are spans,
not moments: Chicago rebuilt, the Panic and its depression, Menlo Park, and the
Indian famine. Menlo Park in particular runs across five later chapters. A
chronological book has to decide, per span chapter, whether it sits at the
span's start, sits at its most consequential moment, or gets broken into
...
constitutional crisis, which serves the book's thesis about a connected
decade. That is a reason to be suspicious of it: the arrangement should follow
...
Each of these has to be decided once and applied everywhere. All are recorded
