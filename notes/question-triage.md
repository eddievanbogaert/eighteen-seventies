...
Editorial, not binding. The categories are mine and the author should overrule
any of them.
...
| | meaning |
| --- | --- |
| **G** — gate | The chapter cannot be drafted until this is answered. Sequence, dates, statutory and primary text, and any rule 4 dispute the chapter has to be *built around* rather than mention. |
| **P** — parallel | Answerable while drafting. The chapter's shape survives without it; leave a marked gap and fill it. |
| **A** — authorial | Not research. A decision only the author can make. |
| **C** — convention | Settled once, applied everywhere. Can be late, must not be lost. |
| **T** — thesis-critical | Overlaid on the above. If this is answered a certain way, it changes *the book*, not a chapter. |
...
and whether the Afghan war was paid for from London or from Indian revenue.
Every one of them is a place where money crosses a border.

This matters because the money thread is currently five American chapters and
the empire thread is five non-American ones, so the threads partition by
geography at exactly the point the connected-decade argument needs them not to.
If these four come back well sourced, the book's structure holds and there is a
strong case for a European money chapter. If they come back thin, the honest
book is more American than its framing currently admits, and the framing should
change rather than the evidence.

**Two of the four have no source in the manifest.** Q01-3 and Q07-5 are well
...
obtainable, as `/primary/README.md` already notes. One push empties this
...
not be closed by buying books. They need repositories, databases, translation,
and in some cases correspondence with people whose reply time is not under
anyone's control.

They therefore have the longest lead time of anything in the project **and** are
the likeliest to be quietly dropped, because each one can be finessed with a
sentence about the limits of the record. Rule "absence of a source is content"
protects the book from pretending; nothing protects the schedule from starting
these too late to succeed. Start them in week one and let them run in the
...
Roughly in order, with the slow things started early rather than done early.
...
This file goes stale the moment questions start closing. When a question becomes
a `CLAIMS.md` row and leaves `QUESTIONS.md`, it should leave the table above
too. If the two files disagree, `QUESTIONS.md` is right and this one is out of
date.
